import Cart from '../models/carrito.js'; // Asegúrate de importar el modelo Cart
...
class CartManager {
    ...
    async addProductToCart(cartId, productId) {
        ...
        await this.collection.updateOne({ _id: new mongoose.Types.ObjectId(cartId) }, { $set: { products: cart.products } }); // Actualiza el carrito en la base de datos
    ...
    async clearProductsInCart(cartId) {
        await this.collection.updateOne({ _id: new mongoose.Types.ObjectId(cartId) }, { $set: { products: [] } });
    ...
    async removeProductFromCart(cartId, productId) {
        ...
        if (!ObjectId.isValid(productId)) { // Asegúrate de usar ObjectId aquí
            throw new Error("ID inválido para el producto.");
        }

        const productIndex = cart.products.findIndex((p) => p.product.equals(new ObjectId(productId))); 
